...
from Train_nuisance import S_coxph, S_rfs, ps_spline, ps_rf #ps_nn, ps_kernel
...
def process_fold(
    ...
    if method_f[0] == 'gam' or method_f[1] == []:
        fit_f = ps_spline(
            simdat,
            train_index,
            test_index,
    ...
    elif method_f[0] == 'rf' and method_f[1] != []:
        fit_f = ps_rf(
            simdat,
            train_index,
            test_index,
    ...
    if method_pi[0] == 'gam' or method_pi[1] == []:
        fit_pi = ps_spline(
            simdat,
            train_index,
            test_index,
    ...
    elif method_pi[0] == 'rf' and method_pi[1] != []:
        fit_pi = ps_rf(
            simdat,
            train_index,
            test_index,
...
def train_nuisance_func(
    ...
    tau=-1,accuracy=500,
    min_val=1e-2,
    method_SD=['rsf',['X1','X2'],100,20],
    method_SC=['rsf',['X1','X2'],100,20],
    method_pi=['gam',['X1','X2']],
    method_f=['gam',['X1','X2']],
):
    n_jobs = Kfold
