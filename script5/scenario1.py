...
for i in range(Looptime):
    ...
    while True:
        ...
        try:
            ...
            with open(os.devnull, "w") as fnull:
                with contextlib.redirect_stdout(fnull), \
                    ...
                    model_true = train_nuisance_func(
                        ...
                        method_SD=['rsf', ['X1', 'X2', 'Z', 'A'], 100, 10],
                        method_SC=['cox', ['Z', 'A', 'X1', 'X2'], 100, 10],
                        method_pi=['gam', ['X1', 'X2']],
                        method_f=['gam', ['X1', 'X2']]
                    )
                    ...
                    model_SD = train_nuisance_func(
                        simdat=simdat, accuracy=200, tau=tau,
                        min_val=0.05, Kfold=5,
                        method_SD=['rsf', ['Z', 'A', 'X1', 'X2'], 100, 10],
                        method_SC=['cox', [], 100, 10],
                        method_pi=['gam', ['X1', 'X2']],
                        method_f=['gam', []]
                    )
            ...
            print(f"{i + 1}/{Looptime} completed.", flush=True)
            break

        except Exception as e:
            print(
                f"Iteration {i + 1} failed on attempt {attempt}: "
                f"{type(e).__name__}: {e}",
                flush=True
            )

            if attempt >= max_retries:
                print(f"Iteration {i + 1} failed after {max_retries} attempts.", flush=True)
                raise

            time.sleep(1)

with open(f'scenario1_{n}.pkl', 'wb') as f:
    pickle.dump(res, f)